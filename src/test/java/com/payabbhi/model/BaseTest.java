...
public class BaseTest {
  ...
  protected void mockFetcher() throws PayabbhiException, IOException {
    ...
    // Retrieve a product
    when(mf.fetch(
            Method.GET, "https://payabbhi.com/api/v1/products/prod_v0RYyTj4qEj56c12", EMPTY_BODY))
    ...
    when(mf.fetch(
            Method.GET,
            "https://payabbhi.com/api/v1/plans?product_id=prod_wJ6DyX5Bgg2LqAqt",
            EMPTY_BODY))
    ...
    // Retrieve a plan
    when(mf.fetch(
            Method.GET, "https://payabbhi.com/api/v1/plans/plan_P7wNUwTdGC2u2n2I", EMPTY_BODY))
    ...
    when(mf.fetch(
            Method.GET,
            "https://payabbhi.com/api/v1/subscriptions?plan_id=plan_tuOWN0Sc0uMB4s8E",
            EMPTY_BODY))
    ...
    when(mf.fetch(
            Method.POST,
            "https://payabbhi.com/api/v1/subscriptions/sub_invalid/cancel",
            EMPTY_BODY))
        .thenReturn(respondWith("/api/v1/exceptions/invalidrequesterror.json", 400));
    ...
    when(mf.fetch(
            Method.DELETE, "https://payabbhi.com/api/v1/invoiceitems/item_invalid", EMPTY_BODY))
        .thenReturn(respondWith("/api/v1/exceptions/invalidrequesterror.json", 400));
    ...
    // Create no invoice when parameters are less then min mandatory params required 1.7
    when(mf.fetch(
            Method.POST,
            "https://payabbhi.com/api/v1/invoices",
    ...
    // Cancel an invoice with invalid ID
    when(mf.fetch(
            Method.POST, "https://payabbhi.com/api/v1/invoices/invc_invalid/cancel", EMPTY_BODY))
        .thenReturn(respondWith("/api/v1/exceptions/invalidrequesterror.json", 400));

    // Retrieve all payment of an invoice with invalid invoice ID
    when(mf.fetch(
            Method.GET, "https://payabbhi.com/api/v1/invoices/invc_invalid/payments", EMPTY_BODY))
        .thenReturn(respondWith("/api/v1/exceptions/invalidrequesterror.json", 400));
    ...
    when(mf.fetch(
            Method.POST,
            "https://payabbhi.com/api/v1/invoices/invc_v9YicJdb67siaXue/cancel",
            EMPTY_BODY))
        .thenReturn(respondWith("/api/v1/invoices/cancelled.json", 200));
    ...
    // Retrieve a transfer
    when(mf.fetch(
            Method.GET, "https://payabbhi.com/api/v1/transfers/trans_ucwszWrXUZJGDgMX", EMPTY_BODY))
    ...
    // Retrieve a event
    when(mf.fetch(
            Method.GET, "https://payabbhi.com/api/v1/events/evt_gCmIpp76zgZynfEO", EMPTY_BODY))
