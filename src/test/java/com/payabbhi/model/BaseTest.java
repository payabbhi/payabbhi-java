...
public class BaseTest {
	...
	protected void mockFetcher() throws PayabbhiException, IOException {
		...
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/payments/pay_zlAsx5g7yH88xcFE/transfers", EMPTY_BODY))
		...
		when(mf.fetch(Method.POST, "https://payabbhi.com/api/v1/subscriptions/sub_xLH108FJwUlX47SI/cancel",
		...
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/invoices?subscription_id=sub_kds3f349dsl4dk9x",
				EMPTY_BODY)).thenReturn(respondWith("/api/v1/invoices/filteredall.json", 200));

		// Retrieve all invoices of an invoiceItem
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/invoiceitems/item_KI75WbBlizfzLZui/invoices",
				EMPTY_BODY)).thenReturn(respondWith("/api/v1/invoices/all.json", 200));
		...
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/invoices/invt_srxOZZk6dIgWTVls/payments", EMPTY_BODY))
				.thenReturn(respondWith("/api/v1/invoices/payments.json", 200));

		// Retrieve all line_items of an invoice
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/invoices/item_FL3nHHB7i7Fpcj1J/line_items", EMPTY_BODY))
		...
		when(mf.fetch(Method.POST, "https://payabbhi.com/api/v1/payments/pay_zlAsx5g7yH88xcFE/transfers",
		...
		when(mf.fetch(Method.POST, "https://payabbhi.com/api/v1/payments/pay_zlAsx5g7yH88xcFE/transfers",
				"{\"transfers\":[{\"beneficiary_id\":\"bene_d7d8b37d264c4264\",\"amount\":20,"
						+ "\"currency\":\"INR\"}]}")).thenReturn(respondWith("/api/v1/transfers/create.json", 200));
		...
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/settlements/stl_wmjBUMNyLnzGbFKU", EMPTY_BODY))
		...
		when(mf.fetch(Method.POST, "https://payabbhi.com/api/v1/payment_links",
		...
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/payment_links/invc_NRFJkTGyZYo03cPD", EMPTY_BODY))
		...
		when(mf.fetch(Method.POST, "https://payabbhi.com/api/v1/payment_links/invc_NRFJkTGyZYo03cPD/cancel",
				EMPTY_BODY)).thenReturn(respondWith("/api/v1/payment_links/cancel.json", 200));

		// Cancel an payment link with invalid ID
		when(mf.fetch(Method.POST, "https://payabbhi.com/api/v1/payment_links/invc_invalid/cancel", EMPTY_BODY))
				.thenReturn(respondWith("/api/v1/exceptions/invalidrequesterror.json", 400));

		// Retrieve all payment of a payment_link with invalid payment_link ID
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/payment_links/invc_invalid/payments", EMPTY_BODY))
				.thenReturn(respondWith("/api/v1/exceptions/invalidrequesterror.json", 400));

		// Retrieve all payments of a payment_link
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/payment_links/invc_NRFJkTGyZYo03cPD/payments",
				EMPTY_BODY)).thenReturn(respondWith("/api/v1/payment_links/payments.json", 200));
		...
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/beneficiaryaccounts/bene_d7d8b37d264c4264", EMPTY_BODY))
		...
		// Create a payment_link with less params
		when(mf.fetch(Method.POST, "https://payabbhi.com/api/v1/beneficiaryaccounts",
		...
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/virtual_accounts/va_FMkEnEGEmHhMKZzL", EMPTY_BODY))
		...
		when(mf.fetch(Method.PATCH, "https://payabbhi.com/api/v1/virtual_accounts/va_FMkEnEGEmHhMKZzL", EMPTY_BODY))
				.thenReturn(respondWith("/api/v1/virtual_accounts/close.json", 200));

		// Retrieve all payment of a virtual_account
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/virtual_accounts/va_PakYenlyIIPjGwoU/payments",
				EMPTY_BODY)).thenReturn(respondWith("/api/v1/virtual_accounts/payments.json", 200));
		...
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/payments/pay_4I4NDogajGtV9bVo/virtual_account",
				EMPTY_BODY)).thenReturn(respondWith("/api/v1/virtual_accounts/virtualaccountdetails.json", 200));

		// Get all Payouts
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/payouts", EMPTY_BODY))
				.thenReturn(respondWith("/api/v1/payouts/all.json", 200));

		// Get payouts with count = 2
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/payouts?count=2", EMPTY_BODY))
				.thenReturn(respondWith("/api/v1/plans/filteredall.json", 200));

		// Retrieve a payout with invalid ID
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/payouts/payout_invalid", EMPTY_BODY))
				.thenReturn(respondWith("/api/v1/exceptions/invalidrequesterror.json", 400));

		// Create a payout with less params
		when(mf.fetch(Method.POST, "https://payabbhi.com/api/v1/payouts", "{\"amount\":1000,\"currency\":\"INR\"}"))
				.thenReturn(respondWith("/api/v1/exceptions/invalidrequesterror.json", 400));

		// Get all payouts will throw error because of invalid params
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/payouts?count=", EMPTY_BODY))
				.thenReturn(respondWith("/api/v1/exceptions/invalidrequesterror.json", 400));

		// Retrieve a payout
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/payouts/pout_DBzMRrGfIQtHoBs7", EMPTY_BODY))
				.thenReturn(respondWith("/api/v1/payouts/pout_DBzMRrGfIQtHoBs7.json", 200));

		// Create a payout
		when(mf.fetch(Method.POST, "https://payabbhi.com/api/v1/payouts",
				"{\"beneficiary_name\":\"BenTest\",\"amount\":1000,\"remittance_account_no\":\"1234567890\",\"method\":\"bank_transfer\",\"beneficiary_ifsc\":\"ABCD1234567\",\"purpose\":\"cashback\",\"narration\":\"info\",\"currency\":\"INR\",\"instrument\":\"NEFT\",\"beneficiary_account_no\":\"01234567890\",\"merchant_reference_id\":\"ref_00001\"}"))
						.thenReturn(respondWith("/api/v1/payouts/create.json", 200));

		// Retrieve a remittance account with invalid ID
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/remittance_accounts/ra_invalid", EMPTY_BODY))
				.thenReturn(respondWith("/api/v1/exceptions/invalidrequesterror.json", 400));

		// Retrieve a remittance account
		when(mf.fetch(Method.GET, "https://payabbhi.com/api/v1/remittance_accounts/123456789012345", EMPTY_BODY))
				.thenReturn(respondWith("/api/v1/remittance_accounts/remittanceaccountdetails.json", 200));
